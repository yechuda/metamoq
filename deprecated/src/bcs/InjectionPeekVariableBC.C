...
template<>
InputParameters validParams<InjectionPeekVariableBC>()
{
...
InjectionPeekVariableBC::InjectionPeekVariableBC(const InputParameters & parameters) :
...
Real
InjectionPeekVariableBC::computeQpResidual()
{
...
Real
InjectionPeekVariableBC::computeQpJacobian()
{
  return _p*_phi[_j][_qp]*_test[_i][_qp];
}

Real
InjectionPeekVariableBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real _E0;
  Real _r0 = _q_point[_qp](_r_axis);
