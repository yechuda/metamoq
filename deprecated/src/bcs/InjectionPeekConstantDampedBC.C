...
InjectionPeekConstantDampedBC::InjectionPeekConstantDampedBC(const InputParameters & parameters) :
...
Real
InjectionPeekConstantDampedBC::computeQpResidual()
{
...
Real
InjectionPeekConstantDampedBC::computeQpJacobian()
{
  return _p*_phi[_j][_qp]*_test[_i][_qp];
}

Real
InjectionPeekConstantDampedBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  RealVectorValue _E_vector(_E_x[_qp], _E_y[_qp], _E_z[_qp]);
  Real _E_n = _E_vector * _normals[_qp];
  Real _E0;
