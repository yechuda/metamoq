...
InputParameters validParams<FaresSchroder>()
{
  ...
  params.addParam<Real>("beta_c", 0.072, "beta_c parameter");
...
FaresSchroder::FaresSchroder(const InputParameters & parameters) :
  Kernel(parameters),
...
Real FaresSchroder::computeQpResidual()
{
  // convection part
  ...
  // destruction part
  // Real psi_k = std::max(0.0, (_nu_tilde[_qp] * _grad_omega[_qp] * _grad_omega[_qp] + omega * _grad_nu_tilde[_qp] * _grad_omega[_qp]) / std::pow(omega, 3.0));
  Real psi_k = std::max(0.0, (_u[_qp] * _grad_omega[_qp] * _grad_omega[_qp] + omega * _grad_u[_qp] * _grad_omega[_qp]) / std::pow(omega, 3.0));
  Real f_beta_star = (1.0 + 650.0 * psi_k) / (1.0 + 100.0 * psi_k);
  Real beta_star = _beta_c_star * f_beta_star;

  RealTensorValue Omega;
  Omega(0,0) = Omega(1,1) = Omega(2,2) = 0.0;
  Omega(0,1) = 0.5 * (_grad_u_vel[_qp](1) - _grad_v_vel[_qp](0));
  Omega(0,2) = 0.5 * (_grad_u_vel[_qp](2) - _grad_w_vel[_qp](0));
  Omega(1,0) = 0.5 * (_grad_v_vel[_qp](0) - _grad_u_vel[_qp](1));
  Omega(1,2) = 0.5 * (_grad_v_vel[_qp](2) - _grad_w_vel[_qp](1));
  Omega(2,0) = 0.5 * (_grad_w_vel[_qp](0) - _grad_u_vel[_qp](2));
  Omega(2,1) = 0.5 * (_grad_w_vel[_qp](1) - _grad_v_vel[_qp](2));

  Real OmegaOmegaS = Omega(0,1) * Omega(1,0) * S(0,0) + Omega(0,1) * Omega(1,2) * S(2,0) + Omega(0,2) * Omega(2,0) * S(0,0) +
                     Omega(0,2) * Omega(2,1) * S(1,0) + Omega(1,0) * Omega(0,1) * S(1,1) + Omega(1,0) * Omega(0,2) * S(2,1) +
                     Omega(1,2) * Omega(2,0) * S(0,1) + Omega(1,2) * Omega(2,1) * S(1,1) + Omega(2,0) * Omega(0,1) * S(1,2) +
                     Omega(2,0) * Omega(0,2) * S(2,2) + Omega(2,1) * Omega(1,0) * S(0,2) + Omega(2,1) * Omega(1,2) * S(2,2);

  Real psi_omega = std::abs(OmegaOmegaS / std::pow(_beta_c_star * omega, 3.0));
  Real f_beta = (1.0 + 70.0 * psi_omega) / (1.0 + 80.0 * psi_omega);
  Real beta = _beta_c * f_beta;

  Real destruction_part = (beta_star - beta) * omega * _u[_qp] * _test[_i][_qp];
...
Real FaresSchroder::computeQpJacobian()
{
  // convection part
  RealVectorValue U(_u_vel[_qp], _v_vel[_qp], _w_vel[_qp]);
  ...
  Real destruction_part = (beta_star - beta) * omega * _phi[_j][_qp] * _test[_i][_qp];
...
Real FaresSchroder::computeQpOffDiagJacobian(unsigned jvar)
{
  ...
  else if (jvar == _omega_var_number)
  {
    ...
    Real destruction_part = (beta_star - beta) * _phi[_j][_qp] * _u[_qp] * _test[_i][_qp];
