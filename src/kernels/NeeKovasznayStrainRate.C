...
NeeKovasznayStrainRate::NeeKovasznayStrainRate(const InputParameters & parameters) :
  Kernel(parameters),
...
Real NeeKovasznayStrainRate::computeQpResidual()
{
  // convection part
  ...
  // production part
  Real S_squared = 2.0 * std::pow(_grad_u_vel[_qp](0), 2.0) + 2.0 * std::pow(_grad_v_vel[_qp](1), 2.0) + 2.0 * std::pow(_grad_w_vel[_qp](2), 2.0) + std::pow(_grad_u_vel[_qp](1) + _grad_v_vel[_qp](0), 2.0) + std::pow(_grad_u_vel[_qp](2) + _grad_w_vel[_qp](0), 2.0) + std::pow(_grad_v_vel[_qp](2) + _grad_w_vel[_qp](1), 2.0);
...
Real NeeKovasznayStrainRate::computeQpJacobian()
{
  // convection part
  RealVectorValue U(_u_vel[_qp], _v_vel[_qp], _w_vel[_qp]);
...
Real NeeKovasznayStrainRate::computeQpOffDiagJacobian(unsigned jvar)
{
