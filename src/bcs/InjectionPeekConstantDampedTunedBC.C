...
#include "Function.h"
registerMooseObject("MetamoqApp", InjectionPeekConstantDampedTunedBC);
...
InputParameters validParams<InjectionPeekConstantDampedTunedBC>()
{
  ...
  params.addParam<Real>("coef", 0.308, "coefficient in the numerator of Peek's formula");

  return params;
}

InjectionPeekConstantDampedTunedBC::InjectionPeekConstantDampedTunedBC(const InputParameters & parameters) :
...
Real
InjectionPeekConstantDampedTunedBC::computeQpResidual()
{
  ...
  if (_geometry == 0)
    _E0 = 3.1e06 * (1 + (_coef/std::sqrt(0.5 * 100 * _r0)));
  else
    _E0 = 3.1e06 * (1 + (_coef/std::sqrt(100 * _r0)));
...
Real
InjectionPeekConstantDampedTunedBC::computeQpJacobian()
{
  return _p*_phi[_j][_qp]*_test[_i][_qp];
}

Real
InjectionPeekConstantDampedTunedBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  RealVectorValue _E_vector(_E_x[_qp], _E_y[_qp], _E_z[_qp]);
  Real _E_n = _E_vector * _normals[_qp];
  Real _E0;
